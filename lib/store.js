...
const DEFAULT_STATE = {
  teams: [
    ...
    {
      id: 'team2',
      name: 'The Deadline Drifters',
      members: [
        { name: 'Yazhini', roll: '714023104176' },
      ],
    },
    {
      id: 'team3',
  ...
  extraVoters: [
    ...
    { name: 'Surya Y', roll: '712523205062' },
    { name: 'Ragav', roll: '712523205013' },
  ],
...
/** Find a voter by roll number. Returns { name, roll } or null. */
export function findVoterByRoll(data, roll) {
  if (!roll) return null;
  const normalized = String(roll).trim();
  const voters = data.extraVoters ?? [];
  const voter = voters.find(v => String(v.roll).trim() === normalized);
  return voter ? { name: voter.name, roll: voter.roll } : null;
}

/** Get all voters with their info. */
...
export async function castVote(data, teamId, voterRoll, score) {
  ...
  // Check if voter already used all votes (max = number of teams)
  const usedVotes = getVoteCount(data, voterRoll);
  const maxVotes = data.teams.length;
